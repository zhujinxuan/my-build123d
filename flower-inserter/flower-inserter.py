...
def mkBase():
    ...
    trimTop = Plane(topf) * (
        base2d - RegularPolygon(radius=radius - thickness / 2, side_count=8)
    )
    return (
        base
        - extrude(innerSketch, amount=thickness - baseHeight)
        - extrude(trimTop, amount=-connectHeight)
    )
...
def mkCover():
    ...
    innerSketch: Sketch = Plane(topf) * RegularPolygon(
        radius=radius - thickness / 2 + 0.5, side_count=8
    )
    polygons = Sketch() + [
    ...
    return (
        cover
        - extrude(innerSketch, amount=-connectHeight)
        - extrude(polygons, thickness)
    )
